...
class Backend_news extends CI_Controller {
 ...
 //Form Validation
        $this->form_validation->set_rules('title', 'News Title', 'required', array('required' => "%s is required"));
        $this->form_validation->set_rules('details', 'News Details', 'required', array('required' => "%s is required"));
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!empty($result[0])) {
                $data['news_image'] = "uploads/news_image/" . $result[0];
            }
            //save slider
